...
const SearchBar: React.FC = () => {
    ...
    <div className="p-4">
      ...
      <div className="mt-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          {['New York', 'Los Angeles', 'Miami', 'San Francisco', 'Chicago', 'Austin'].map((city) => (
            <div 
              key={city} 
  ...
  const DateTab: React.FC<{type: 'checkIn' | 'checkOut'}> = ({type}) => (
    <div className="p-4">
      <h3 className="font-bold text-lg mb-2">{type === 'checkIn' ? 'Check-in date' : 'Check-out date'}</h3>
      <div className="flex items-center border rounded-lg p-3 bg-gray-100">
        ...
        <input
          ...
          onChange={(e) => type === 'checkIn' ? setCheckIn(e.target.value) : setCheckOut(e.target.value)}
          autoFocus
        />
      </div>
    </div>
  ...
  return (
    ...
    <div className="fixed inset-0 bg-[#918f8f97] bg-opacity-25 flex justify-center items-center z-50">
      <div className="w-full max-w-3xl mx-auto bg-white rounded-lg shadow-2xl" ref={searchBarRef}>
        {/* Tabs */}
        <div className="flex border-b">
          <button 
            className={`flex-1 py-6 text-center font-medium  text-base ${activeTab === 'location' ? 'border-b-2 border-black' : ''}`}
            onClick={() => handleFocus('location')}
          >
            Where
          </button>
          <button 
            className={`flex-1 py-6 text-center font-normal text-sm ${activeTab === 'checkIn' ? 'border-b-2 border-black' : ''}`}
            onClick={() => handleFocus('checkIn')}
          >
            Check in
          </button>
          <button 
            className={`flex-1 py-6 text-center font-normal text-sm  ${activeTab === 'checkOut' ? 'border-b-2 border-black' : ''}`}
            onClick={() => handleFocus('checkOut')}
          >
            Check out
          </button>
          <button 
            className={`flex-1 py-6 text-center font-normal text-sm  ${activeTab === 'guests' ? 'border-b-2 border-black' : ''}`}
